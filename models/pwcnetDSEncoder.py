...
from .pwc_modules import WarpingLayer, ContextNetwork, FlowEstimatorDense
from .correlation_package.correlation import Correlation


class DownSamplePyramid(nn.Module):
    def __init__(self, num_chs):
        super(DownSamplePyramid, self).__init__()
        self._depth = len(num_chs) - 1
    ...
    def forward(self, x):
        ...
        for i in range(self._depth):
            _, _, h, w = x.size()
            x = tf.adaptive_avg_pool2d(x, [h//2, w//2])
            feature_pyramid.append(x)

        return feature_pyramid[::-1]


class PWCNetDSEncoder(nn.Module):
    def __init__(self, args, div_flow=0.05):
        super(PWCNetDSEncoder, self).__init__()
        self.args = args
        self._div_flow = div_flow
        self.search_range = 4
        self.num_chs = [3, 3, 3, 3, 3, 3, 3]  # RGB we pass the (downsampled) RGB images only
        self.output_level = 4
        self.num_levels = 7
        self.leakyRELU = nn.LeakyReLU(0.1, inplace=True)

        self.feature_pyramid_extractor = DownSamplePyramid(self.num_chs)
        ...
        self.context_networks = ContextNetwork(536)
