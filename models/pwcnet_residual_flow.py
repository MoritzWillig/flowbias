...
class PWCNetResidualFlow(nn.Module):
    
    def __init__(self, args, div_flow=0.05):
        super(PWCNetResidualFlow, self).__init__()
        self.args = args
    ...
    def forward(self, input_dict):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            # flow estimator
            if l == 0:
                x_intm, flow_res = self.flow_estimators[l](out_corr_relu)
                flow += flow_res
            else:
                x_intm, flow_res = self.flow_estimators[l](torch.cat([out_corr_relu, x1, flow], dim=1))
                flow += flow_res
