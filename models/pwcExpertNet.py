...
class FeatureExtractorExpert(nn.Module):
    def __init__(self, num_chs, num_experts, expert_split):
        ...
        """
        super(FeatureExtractorExpert, self).__init__()
...
class FlowEstimatorDenseExpert(nn.Module):

    def __init__(self, ch_in, num_experts, expert_split):
        super(FlowEstimatorDenseExpert, self).__init__()
...
class PWCExpertNet(nn.Module):
    def __init__(self, args, num_experts, expert_split, div_flow=0.05):
        ...
        self.feature_pyramid_extractor = FeatureExtractorExpert(self.num_chs, num_experts, expert_split)
        ...
        for l, ch in enumerate(self.num_chs[::-1]):
            ...
            layer = FlowEstimatorDenseExpert(num_ch_in, num_experts, expert_split)
            self.flow_estimators.append(layer)

        self.context_networks = ContextNetworkExpert(self.dim_corr + 32 + 2 + 448 + 2, num_experts, expert_split)
