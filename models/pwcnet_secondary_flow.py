...
class PWCNetSecondaryFlow(nn.Module):
    
    def __init__(self, args, div_flow=0.05):
        super(PWCNetSecondaryFlow, self).__init__()
        self.args = args
        ...
        self.dim_corr = 2 * ((self.search_range * 2 + 1) ** 2)
    ...
    def forward(self, input_dict):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            # flow estimator
            if l == 0:
                x_intm, flow, flow_sec = self.flow_estimators[l](torch.cat([out_corr_relu, out_corr_sec_relu]))
            else:
                x_intm, flow, flow_sec = self.flow_estimators[l](torch.cat([out_corr_relu, out_corr_sec_relu, x1, flow], dim=1))
