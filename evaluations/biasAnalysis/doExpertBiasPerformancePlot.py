...
#results = {key: value for key, value in get_eval_summary().items() if re.match("expertWOX1_CTSK_split02_..$", key)}

#performance_metric = "mean_normalized_performance"
performance_metric = "mean_adjusted_normalized_compensated_performance"
#performance_metric = "inversed_mean_adjusted_normalized_compensated_performance"


plt.figure()
xs = np.array([float(value["normalized_dataset_difference"]) for value in results.values()])
ys = np.array([float(value[performance_metric]) for value in results.values()])
